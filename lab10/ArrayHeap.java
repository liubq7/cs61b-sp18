...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        while (parentIndex(index) != min(index, parentIndex(index)) && index > 1) {
    ...
    private void sink(int index) {
        ...
        while (leftIndex(index) <= size) {
            int smallIndex = min(leftIndex(index), rightIndex(index));
            if (smallIndex == min(index, smallIndex)) {
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i <= size; i++) {
            ...
            if (node.myItem.equals(item)) {
                double prePriority = node.myPriority;
